...
class Worker(threading.Thread):
    ...
    def _do_arm(self, live):
        if live:
            ...
            self.g.disarm()
            self.emit(
                "status", (f"DRY RUN  ·  firmware {self.g.version}  ·  motion is SIMULATED", AMBER)
            )
...
class App:
    ...
    def _build(self):
        ...
        for r, a in enumerate(AXES, start=2):
            tk.Label(root, text=a, bg=BG, fg=FG, font=("TkFixedFont", 16, "bold")).grid(
                row=r, column=0, **pad
            )

            self.pos_lbl[a] = tk.Label(
                root,
                text="+0.000 mm",
                bg=BG,
                fg=GREEN,
            ...
            ttk.Button(
                root, text="Move ±", width=8, command=lambda ax=a: self.on_move(ax, absolute=False)
            ).grid(row=r, column=4, **pad)
            ttk.Button(
                root, text="Go to", width=7, command=lambda ax=a: self.on_move(ax, absolute=True)
